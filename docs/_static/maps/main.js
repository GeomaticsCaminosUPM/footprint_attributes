// Interactive pilot-region map -- one dataset per pilot region, colored
// either by the geometric relative-position classification
// (footprint_attributes.position), the 4-category shape index, or one of 15
// raw, physical-unit seismic-code shape/plan-irregularity metrics
// (footprint_attributes.shape -- EC8, ASCE7, GNDT-II, CSCR-2010, NTC-23,
// slenderness). Norm limits live in footprint_attributes/config.py and are
// mirrored into NORM_INFO below. Generated by
// footprint_attributes.visualization.build_map -- see that module's
// docstring, not this file, to regenerate.
...
// ---------------------------------------------------------------------------
// Norm limits -- see footprint_attributes/config.py for the source of every
// limit below. `worseIsHigh: false` marks the 4 inverted-direction
// parameters (higher value = better): EC8 radiusRatio, EC8 compactness, and
// the two GNDT-II betas that use an inscribed-circle "how well does the
// building fill its own footprint" construction.
const NORM_INFO = {
...
// Categorical attributes -- relative position (geometric contact-force
// classification) and the shape index (built from 3 of the 15 raw metrics,
// see shapeIndexOf below). Both render as a swatch legend + count bar chart
// instead of a green/red norm gradient.
...
const CATEGORICAL_INFO = {
  relativePosition: { key: positionKey, colors: POSITION_COLORS, order: POSITION_ORDER, labels: POSITION_LABELS },
  shape_index: { key: (f) => shapeIndexOf(f), colors: FSI_COLORS, order: FSI_ORDER, labels: FSI_LABELS },
};

// Video-cycle order, per the brief: relative position first, then the shape
// index, then round-robin across code families (one slenderness, one GNDT,
// one ASCE7, one EC8, one NTC, one Costa Rica, repeat) so two columns from
// the same code never show back to back. This *is* the family order the
// "Color by" dropdown uses too.
const ATTRIBUTES = [
  { name: "relativePosition", label: "Relative position" },
...
// The "Color by" dropdown lists the same attributes grouped by family
// (relative position, then shape index, then EC8, then ASCE 7, then
// GNDT-II, then Costa Rica, then NTC-23) -- a different, easier-to-scan
// order than the video's family-interleaved cycle above (ATTRIBUTES itself
// stays untouched for that).
const DROPDOWN_FAMILY_ORDER = ["relativePosition", "shape_index", "EC8", "slenderness", "ASCE7", "GNDTII", "CSCR2010", "NTC23"];
function attributeFamily(name) {
  if (name === "relativePosition" || name === "shape_index") return name;
  if (name.startsWith("slenderness")) return "slenderness";
  const match = name.match(/^([A-Za-z0-9]+?)_/);
  return match ? match[1] : name;
}
const DROPDOWN_ATTRIBUTES = [...ATTRIBUTES].sort((a, b) => {
...
// Overlay geometry layers -- convex hull / bounding box (polygons, drawn as
// unfilled outlines) and inertia axis / basic-length dimensions / net
// contact-force resultant (arrow/dimension LineStrings, built by
// footprint_attributes.notebook_utils.arrow_gdf and friends -- see
// footprint_attributes.visualization.maps for how each file is written).
// All start off; toggled independently from the "Overlays" checkboxes.
const OVERLAY_DEFS = [
  { id: "convex_hull", label: "Convex hull", kind: "polygon", color: "#4299e1" },
  { id: "bounding_box", label: "Bounding box", kind: "polygon", color: "#e2a33f" },
  { id: "inertia_axis", label: "Inertia axis", kind: "line", color: "#e2938a" },
  { id: "basic_lengths", label: "Building lengths (L1/L2/a1/a2/b/c)", kind: "line", color: "#111111" },
  { id: "position_arrows", label: "Position force arrows", kind: "line", color: "#8744ad" },
];
...
function getFillColor(feature) {
  ...
  if (categorical) {
    return [...hexToRgb(categorical.colors[categorical.key(feature)]), 210];
  }
...
function buildingTooltip({ object, layer }) {
  ...
  const categorical = CATEGORICAL_INFO[state.attribute.name];
  if (categorical) {
    const label = categorical.labels[categorical.key(object)];
...
function renderLegend() {
  ...
  if (categorical) {
    const counts = {};
    for (const f of state.data.features) counts[categorical.key(f)] = (counts[categorical.key(f)] ?? 0) + 1;
...
// ---------------------------------------------------------------------------
// Categorical count chart -- used for both relativePosition and shape_index
// (always visible below whatever norm chart is active, when applicable).
...
function renderCategoricalChart() {
  ...
  const card = document.getElementById("shape-index-chart-card");
  const categorical = CATEGORICAL_INFO[state.attribute.name];
  if (!categorical) {
    card.classList.add("hidden");
    container.innerHTML = "";
  ...
  card.classList.remove("hidden");
  document.getElementById("shape-index-chart-title").textContent = `${state.attribute.label} per building`;
  ...
  entries.forEach((key, i) => {
    ...
    labels += `<text x="${cx + 4}" y="${plotBottom + 12}" text-anchor="end" class="chart-axis-label" transform="rotate(-40 ${cx + 4} ${plotBottom + 12})">${categorical.labels[key].replace("Irregular: ", "")}</text>`;
  });
...
// ---------------------------------------------------------------------------
// Building popup -- relative position, shape index, and every raw shape
// metric for this building (not just the currently-selected "Color by" one).
function renderBuildingPanel(id, properties) {
  ...
  for (const name of ["relativePosition", "shape_index"]) {
    const categorical = CATEGORICAL_INFO[name];
    const row = document.createElement("tr");
    const th = document.createElement("th");
    th.textContent = ATTRIBUTES.find((a) => a.name === name).label;
    const td = document.createElement("td");
    td.textContent = categorical.labels[categorical.key({ properties })];
    row.append(th, td);
    tbody.appendChild(row);
  }

  for (const { name, label } of ATTRIBUTES) {
    if (CATEGORICAL_INFO[name]) continue;
...
// ---------------------------------------------------------------------------
// Overlay checkboxes -- convex hull / bounding box / inertia axis / basic
// lengths / position force arrows, each an independent on/off toggle.
...
// next attribute in ATTRIBUTES's own order (relative position, then shape
// index, then family-interleaved shape metrics).
